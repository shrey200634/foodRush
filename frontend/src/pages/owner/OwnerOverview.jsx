...
import {
  TrendingUp, ShoppingBag, Clock, ChefHat, ArrowRight,
...
export default function OwnerOverview() {
  ...
  const handleToggleOpen = async () => {
    ...
    try {
      ...
      toast.success(updated.isOpen ? "Restaurant is now open! 🟢" : "Restaurant closed 🔴");
  ...
  if (loading && restaurants.length === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center",
        ...
        <p style={{ color: INK_MUTED, fontSize: "0.9rem" }}>Loading your restaurant...</p>
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      </div>
    );
  }

  if (!loading && restaurants.length === 0) return <NoRestaurantSetup />;
  ...
  return (
    <div style={{ color: INK, animation: "fade-up 0.4s ease-out" }}>
      ...
      <div style={{ marginBottom: 36, display: "flex", alignItems: "flex-start",
        ...
        <div>
          ...
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.8rem,3vw,2.8rem)",
            fontWeight: 500, letterSpacing: "-0.03em", lineHeight: 1, color: INK }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))",
        ...
        {[
          { label: "Revenue Today",  value: `₹${revenue.toFixed(0)}`,  icon: TrendingUp, color: SAFFRON,
            sub: `${delivered.length} orders delivered` },
          { label: "Active Orders",  value: active.length,              icon: Clock,      color: "#A78BFA",
            sub: "Needs attention" },
          { label: "Total Orders",   value: orders.length,              icon: ShoppingBag, color: TC_SOFT,
            sub: `${cancelled.length} cancelled` },
          { label: "Menu Items",     value: menuItems.length,           icon: ChefHat,    color: PISTACHIO,
            sub: r?.isPureVeg ? "Pure Veg ✓" : "Veg + Non-veg" },
        ].map((s, i) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ background: CARD, borderRadius: 20, padding: "24px",
          border: `1px solid ${INK_HAIR}` }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {activeOrders.slice(0, 4).map((order) => (
                <div key={order.orderId} style={{ display: "flex", alignItems: "center",
                  ...
                  border: ["PLACED","CREATED"].includes(order.status)
                    ? `1px solid ${TC}40` : `1px solid ${INK_HAIR}` }}>
                  <div>
                    <div style={{ fontSize: "0.88rem", fontWeight: 700, color: INK }}>
                      #{order.orderId?.slice(-6).toUpperCase()}
                      {["PLACED","CREATED"].includes(order.status) && (
                        <span style={{ marginLeft: 8, fontSize: "0.62rem",
                          background: `${TC}20`, color: TC_SOFT,
        ...
        <div style={{ background: CARD, borderRadius: 20, padding: "24px",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {[
              ...
              { label: "Delivery time", value: `${r?.avgDeliveryTimeMins || 30} mins` },
              { label: "Min. order",    value: r?.minOrderAmount > 0 ? `₹${r.minOrderAmount}` : "No minimum" },
              { label: "Delivery fee",  value: r?.deliveryFee > 0 ? `₹${r.deliveryFee}` : "Free delivery" },
              { label: "Type",          value: r?.isPureVeg ? "🌿 Pure Veg" : "🍖 Veg + Non-veg" },
            ].map((f, i, arr) => (
...
function NoRestaurantSetup() {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "70vh" }}>
      <div style={{ textAlign: "center", maxWidth: 440, color: "#FFF5E6" }}>
        <div style={{ fontSize: "4rem", marginBottom: 20 }}>🍽</div>
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: "2rem", marginBottom: 12 }}>
          Set up your restaurant
        </h2>
        <p style={{ color: "rgba(255,245,230,0.45)", lineHeight: 1.7, marginBottom: 32 }}>
          You don't have a restaurant registered yet. Contact the FoodRush support team to get onboarded.
        </p>
      </div>
    </div>
  );
