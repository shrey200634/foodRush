...
import { MapPin, Plus, CheckCircle, Loader2, ArrowLeft, ShoppingBag, Sparkles, Navigation, CreditCard } from "lucide-react";
...
function OrderSuccess({ orderId, onTrack, onHome }) {
  ...
  const steps = ["Order placed!", "Restaurant notified", "Finding driver..."];
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", animation: "fade-up 0.4s ease-out" }}>
      ...
      <div className="checkout-grid" style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 32, alignItems: "start" }}>
        ...
        <div style={{ background: CARD, borderRadius: 18, border: `1px solid ${INK_HAIR}`,
          ...
          <div style={{ marginTop: 16, padding: "10px 14px", borderRadius: 10, background: FIELD,
            display: "flex", alignItems: "center", gap: 8, fontSize: "0.78rem", color: INK_SOFT }}>
            <CreditCard size={14} style={{ color: TC }} />
            Payment via FoodRush Wallet
          </div>

          <button onClick={handlePlaceOrder} disabled={placing || addresses.length === 0} style={{
